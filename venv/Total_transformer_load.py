...
index = range(0,T,1)
columns = ['Total load']
df_total_base_load = pd.DataFrame(index=index, columns=columns)
...
figure = df_total_base_load_before.loc[:,'Total load'].T.plot(label='Before')
figure = df_total_base_load.loc[:,'Total load'].T.plot(label='After')
figure.legend(loc='best')
figure.set_xlabel("Time")
figure.set_ylabel("Total Load")
figure.set_title("Total Load Variation")
plt.show()
