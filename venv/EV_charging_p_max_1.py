...
# df_base_load[transformer] : An array of DataFrames that holds base load of each transformer
...
# Create a dataframe of base loads for each transformer
df_base_load = {}
for transformer in range(1, m + 1):
    df_base_load[transformer] = (df_base_load_collection[df_base_load_collection['Transformer'] == transformer])[
        ['Time', 'Load']]
...
for ev in range(1, N + 1):
    ...
    for t_start in range(t_plugin[ev], t_dead[ev] - t_length[ev] + 1):
        ...
        for t in range(0, T):
            if t < t_start:
                total_load_before_plugin = (df_base_load[transformer[ev]].iloc[t]['Load']) ** 2
            elif t_start <= t <= (t_start + t_length[ev] - 1):
                total_load_while_plugin = (p_max[ev] + df_base_load[transformer[ev]].iloc[t]['Load']) ** 2
            elif t >= (t_start + t_length[ev]):
                total_load_after_plugin = (df_base_load[transformer[ev]].iloc[t]['Load']) ** 2
    ...
    for t in range(df_ev.loc[ev,'Optimal start time'], t_start + t_length[ev] - 1):
        df_base_load[transformer[ev]].iloc[t]['Load'] += p_max[ev]
